...
export function formatOwnerForExcel(ownerName = '', ownerId = '') {
  const name = normalizedExcelText(ownerName);
  const id = normalizedExcelText(ownerId).toUpperCase();
  if (!name) return id;
  if (!id) return name;

  if (name.toUpperCase().endsWith(id)) return name;

  const isPersonalId = /^[A-Z][12][0-9*]{7}[0-9]$/.test(id);
  if (!isPersonalId) return `${name}${id}`;

  const maskedSurname = name.match(/^([^*]+)\*+/)?.[1];
  if (maskedSurname) return `${maskedSurname}${id}`;

  return `${Array.from(name)[0]}${id}`;
}
